...
/*
    New project 생성 시 주의점 :
        ...
        6. 앞으로의 수업 시간에는 new project 생성 시에 이렇게 자세히 풀이할 일은 자주 있지 않을 예정이니 꼭
...
/*
    1. Spring vs. SpringBoot
        ...
        SpringBoot의 등장
            ...
            즉, 스프링 부트라는 것은 기본적으로 스프링에 속해있는 도구

            차이점 :
                1) 구성의 차이 -> 스프링은 애플리케이션 개발에 필요한 환경을 수동으로 구성하고 정의하지만
            ...
            2. 스프링 컨셉
                1) 제어의 역전(IoC)
                    Inversion of Control : 다른 객체를 직접 생성하거나 제어하는 것이 아니라
                        ...
                        위와는 다르게 클래스 B객체를 직접 생성하는 것이 아니라 어딘가에서 받아와 사용
                ...
                2) 의존성 주입(DI)
                    ...
                    Bean : 스프링 컨테이너가 관리하는 객체 -> 추후 한 번 더 수업할 에정
                ...
                3) 빈과 스프링 컨테이너
                    ...
                    3)-2. 빈 : 스프링 컨테이너가 생성하고 관리하는 객체 -> 이상의 코드들에서 B가 빈에 해당
                        스프링은 빈을 스프링 컨테이너에 등록하기 위해 XML 파일 설정, 애너테이션 추가 드으이
                        ...
                        public class MyBean{}
                            ...
                            소문자로 바꾸어서 관리함. 즉 클래스 MyBean의 이름은 myBean이 됨.

                            일반적으로는 스프링이 제공해주는 객체로 받아들이시면 됩니다.

                4) 관점 지향 프로그래밍(AOP)
                    Aspect-oriented Programmin : 프로그래밍에 대한 관심을 핵심 관점 / 부가 관점으로 나누어 모듈화함을 의미

                        ex) 계좌 이체, 고객 관리하는 프로그램이 있다고 가정할 때, 각 프로그램에는 로깅 조직, 즉 지금까지
                        일어난 일을 기록하기 위한 로직과 여러 데이터를 관리하기 위한 데이터베이스 연결 로직이 포함됨 이 때,
                            핵심 관점 - 계좌 이체, 고객 관리 로직
                            부가 관점 - 로깅, 데이터 베이스 연결 로직

                5) 이식 가능한 서비스 추상화(PSA)
                    Portable Service Abstraction : 스프링에서 제공하는 다양한 기술들을 '추상화' 해 개발자가 쉽게
                        사용하는 '인터페이스'
                        ...
                        WAS 역시 PSA의 예시 중 하나로, 어떤 서버를 사용하더라도(톰캣, 언더토우, 네티 등) 코드를 동일하게 작성 가능
            ...
            3. 스프링 부트 3 둘러보기
                ...
                스프링 부트 스타터 살펴보기
                    ...
                    spring-boot-starter-actuator : 모니터링을 위해 애플리케이션에서 제공하는 다양한 정보를 제공하기 쉽게
                        하는 의존성 모음
